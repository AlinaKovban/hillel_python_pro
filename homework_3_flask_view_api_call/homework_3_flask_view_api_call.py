...
def generate_students():
    counter = int(request.args.get("input_counter", "1000"))
    students = []

    if counter > 1000:
        return "Counter of students should be equal 1000 or less"

    for _ in range(int(counter)):
        birthday = fake_data.date_of_birth(minimum_age=18,
                                           maximum_age=60).strftime("%Y-%m-%d")
...
def get_bitcoin_value():
    ...
    if data_from_url.status_code != HTTPStatus.OK:
        return Response(
